...
namespace Bootstrap.Controllers
{
    public class TreeViewController : Controller
    {
        ...
        public ActionResult UploadFilesToDirectory(string directoryPath)
        {
            try
            {
                ...
                for (int i = 0; i < files.Count; i++)
                {
                    ...
                    var fv = new FileViewModal();
                    fv.FileName = file.FileName;
                    fv.FileType = Path.GetExtension(file.FileName);
                    fv.LastUpdated = DateTime.Now;
                    fv.ActualSize = file.ContentLength;
                    fv.Path = fileNameSave;
                    string[] filePaths = fileNameSave.Split(new[] { AppDomain.CurrentDomain.BaseDirectory }, StringSplitOptions.None);
                    fv.ServerPath = filePaths[1];
                    fileView.Add(fv);
                }
                if (TempData["TempDataFiles"] != null)
                {
                    var oldTempData =  (List<FileViewModal>)TempData["TempDataFiles"];
                    oldTempData.AddRange(fileView);

                    TempData["TempDataFiles"] = oldTempData;
                }
                else {
                    TempData["TempDataFiles"] = fileView;
                }
                return Json(new { isSuccess = true, successMessage = files.Count + " Files Uploaded!", directoryPath = directoryPath });
            }
            catch (Exception ex)
        ...
        public ActionResult UploadedFilesGrid(string directoryPath)
        {
            ...
            var tempDataFiles = (List<FileViewModal>)TempData["TempDataFiles"];
            return PartialView(tempDataFiles.OrderByDescending(x=>x.LastUpdated).ToList());
        }
            ...
            try
            {
                foreach (var file in files)
                {
                    ...
                    if (System.IO.File.Exists(filePath))
                    {
                        ...
                        if (TempData["TempDataFiles"] != null)
                        {
                            var tempFiles = (List<FileViewModal>)TempData["TempDataFiles"];
                            var removeFile = tempFiles.Where(x => x.Path == filePath).FirstOrDefault();
                            if (removeFile != null)
                            {
                                tempFiles.Remove(removeFile);
                               
                            }
                            TempData["TempDataFiles"] = tempFiles;
                        }
                    }
                }
                return true;
